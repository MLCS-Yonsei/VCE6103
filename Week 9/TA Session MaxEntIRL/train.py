...
def idx_demo(env, one_feature):
    ...
    demonstrations = np.zeros((len(raw_demo), len(raw_demo[0]), 2))
...
def idx_state(env, state):
    env_low = env.observation_space.low #[-1.2, -0.07]
    env_high = env.observation_space.high #[0.6, 0.07]
    env_distance = (env_high - env_low) / one_feature #[0.09, 0.007]
...
def main():
    ...
    for episode in range(episode_num):
        ...
        if episode % 1000 == 0:
            ...
            pylab.savefig("./learning_curves/maxent_{0}.png".format(episode_num))
